...
export function runTransactionTests() {
  describe("TRANSACTION ROUTES", () => {
    ...
    beforeEach(async () => {
      ...
      await new Hunter(hunterSample2).save();
    ...
    describe("GET /transactions/by-name", () => {
      ...
      test("Should return 400 if name query is missing in /by-name", async () => {
        const res = await request(app).get("/transactions/by-name").expect(400);
        expect(res.body.error).toBe("You must provide a name in the query string.");
      });
    ...
    describe("GET /transactions/by-date", () => {
      ...
      test("Should return 400 if invalid transaction type is provided", async () => {
        const startDate = new Date(Date.now() - 1000 * 60 * 60).toISOString();
        const endDate = new Date(Date.now() + 1000 * 60 * 60).toISOString();
      
        const res = await request(app)
          .get(`/transactions/by-date?start=${startDate}&end=${endDate}&type=invalido`)
          .expect(400);
      
        expect(res.body.error).toBe('Transaction type must be "purchase", "sell", or "all".');
      });
    ...
    describe("POST /transactions", () => {
      ...
      test("Should return 400 if neither hunterName nor merchantName is provided", async () => {
        const res = await request(app)
          .post("/transactions")
          .send({ goods: [{ name: "Silver Sword", quantity: 1 }] })
          .expect(400);
        expect(res.body.error).toBe("You must specify a hunter or a merchant.");
      });
      test("Should return 404 if only hunterName is provided but not found", async () => {
        const res = await request(app)
          .post("/transactions")
          .send({
            hunterName: "NoExiste",
            goods: [{ name: "Silver Sword", quantity: 1 }],
          })
          .expect(404);
        expect(res.body.error).toBe("Hunter not found.");
      });
