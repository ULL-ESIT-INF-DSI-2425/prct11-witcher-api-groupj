...
export function runGoodTests() {
  describe("GOOD ROUTES", () => {
    ...
    describe("PATCH /goods", () => {
      ...
      test("Should return 404 if no goods match the PATCH query", async () => {
        await request(app)
          .patch("/goods?name=NoExiste")
          .send({ value: 999 })
          .expect(404);
      });
    ...
    describe("DELETE /goods/:id", () => {
      ...
      test("Should return 404 if trying to PATCH a good by nonexistent ID", async () => {
        await request(app)
          .patch("/goods/000000000000000000000000")
          .send({ value: 123 })
          .expect(404);
      });
    });
